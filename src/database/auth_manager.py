...
from src.database.firebase_client import get_firestore_client as get_db
...
# ... (весь твой текущий код в users_db.py) ...

def get_all_users():
    """
    Получает всех пользователей из базы.
    ⚠️ Осторожно: при большом количестве пользователей это может быть медленно.
    Для продакшена лучше использовать пагинацию или поиск по конкретным ID.
    """
    db = get_db()
    if not db:
        return []
